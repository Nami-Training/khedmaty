...
class SliderController extends Controller
{
    ...
    public function getAll(SliderService $sliderService)
    {
        ...
        ->addColumn('action', function($slider){
            return '<button type="button" class="btn btn-primary editButton" data-bs-toggle="modal" data-bs-target="#createModal" model-route="'.route('sliders.edit', $slider->id).'" model-title=" "> <i class="fa fa-pencil"></i> </button>
            <button type="button" class="btn btn-danger deleteButton"  delete-route="'.route('sliders.destroy', $slider->id).'"> <i class="fa fa-trash-o"></i> </button>';
        })
    ...
    {
        $html = '<form class="row g-3 from-submit-global" method="post" action="'.route('sliders.store').'" " enctype="multipart/form-data"> '.csrf_field(). '
            <div class="col-6">
    ...
    public function edit(string $id, SliderService $sliderService)
    {
        ...
        $html = '<form class="row g-3 from-submit-global" method="post" action="'.route('sliders.update', $slider->id).'" " enctype="multipart/form-data"> '.csrf_field(). '' .method_field('put').'
            <div class="col-6">
                <label for="TextInput" class="form-label">Title</label>
