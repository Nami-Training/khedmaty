                ...
                <div class="card-body" style="min-height: calc(100vh - 196px);">
                    <div class="d-flex mb-3 justify-content-end">
                        <button type="button" class="btn btn-success float-start addButton mx-1" data-bs-toggle="modal"
                            data-bs-target="#createModal" model-title="Add Store Category" model-route="{{route('storeCategoies.create')}}"
                            title="Add">
                            <i class="fa fa-plus-circle"></i> Add Store Category
                        </button>
                    </div>
    ...
    <script>
        $(document).ready(function () {
            $('#dataTableObject').DataTable({
                ...
                ajax: '{{ route('storeCategoies.data') }}', // Route for server-side processing
                columns: [
                    { data: 'id' },
                    { data: 'name' },
                    { data: 'description' },
